...
.sn-item { display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:8px;color:#6b7280;font-size:13px;font-weight:500;cursor:pointer;text-decoration:none;transition:.15s; }
.sn-item:hover { background:#f9fafb;color:#374151; }
.sn-item.active { background:#eff6ff;color:#2563eb;font-weight:600; }
.sn-item i { width:18px;text-align:center;font-size:13px; }
.settings-card { background:#fff;border-radius:12px;border:1px solid #f0f0f0;padding:28px; }
.s-title { font-size:15px;font-weight:700;color:#0f1d35;margin-bottom:4px; }
.s-desc  { font-size:12px;color:#6b7280;margin-bottom:20px; }
.s-divider { border-top:1px solid #f0f0f0;margin:20px 0; }
...
.notif-row { display:flex;align-items:center;justify-content:space-between;padding:12px 0;border-bottom:1px solid #f9fafb; }
...
<div class="row g-3">
    ...
    <div class="col-md-3">
        <div class="settings-nav">
            <a href="#" class="sn-item active" onclick="showSection('company',this);return false"><i class="fas fa-building"></i> Company Profile</a>
            <a href="#" class="sn-item" onclick="showSection('general',this);return false"><i class="fas fa-sliders-h"></i> General</a>
            <a href="#" class="sn-item" onclick="showSection('notif',this);return false"><i class="fas fa-bell"></i> Notifications</a>
            <a href="#" class="sn-item" onclick="showSection('pipeline',this);return false"><i class="fas fa-filter"></i> Pipeline Stages</a>
            <a href="#" class="sn-item" onclick="showSection('about',this);return false"><i class="fas fa-info-circle"></i> About</a>
        </div>
    </div>
    ...
    <div class="col-md-9">
            ...
            <form action="{{ route('settings.update') }}" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="company_website" class="form-control" value="{{ $settings['company_website'] ?? '' }}" placeholder="https://example.com">
                    </div>
                    <div class="col-12">
            ...
            <form action="{{ route('settings.update') }}" method="POST">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <div style="font-size:11px;color:#9ca3af;margin-top:3px">Mata uang default untuk revenue dan deal value</div>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Timezone</label>
                        <select name="timezone" class="form-select">
                            @foreach(['Asia/Jakarta'=>'WIB (UTC+7)','Asia/Makassar'=>'WITA (UTC+8)','Asia/Jayapura'=>'WIT (UTC+9)'] as $tz => $label)
                            <option value="{{ $tz }}" @selected(($settings['timezone']??'Asia/Jakarta')==$tz)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
        ...
        <div class="settings-card d-none" id="sec-pipeline">
            ...
            @php
            $stages = [
                ['Identifying','#3b82f6','10%'],['Approaching','#10b981','30%'],
                ['Follow Up','#f59e0b','50%'],['Closing','#f97316','80%'],['Won','#16a34a','100%'],
            ];
            @endphp
            @foreach($stages as $s)
            <div class="d-flex align-items-center gap-3 mb-3 p-3" style="background:#f9fafb;border-radius:8px;border:1px solid #f0f0f0">
                ...
                <span style="font-size:11px;padding:2px 8px;border-radius:20px;background:#f3f4f6;color:#6b7280">Default</span>
            </div>
            @endforeach
        ...
        <div class="settings-card d-none" id="sec-about">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <div style="background:#f9fafb;border-radius:8px;padding:16px">
                        <div style="font-size:11px;color:#9ca3af;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Tech Stack</div>
                ...
                <div class="col-md-6">
                    <div style="background:#f9fafb;border-radius:8px;padding:16px">
                        <div style="font-size:11px;color:#9ca3af;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Logged In As</div>
                        <div style="font-size:16px;font-weight:700;color:#0f1d35;margin-top:4px">{{ auth()->user()->name }}</div>
                        <div style="font-size:12px;color:#6b7280">{{ auth()->user()->email }} · {{ auth()->user()->role }}</div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div style="background:#f9fafb;border-radius:8px;padding:16px">
                        <div style="font-size:11px;color:#9ca3af;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Company</div>
                        <div style="font-size:16px;font-weight:700;color:#0f1d35;margin-top:4px">{{ $settings['company_name'] ?? '-' }}</div>
