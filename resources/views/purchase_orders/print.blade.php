...
<main class="page">
    <header class="letterhead split-letterhead">
        ...
        <div class="split-company-info">
            @if(!empty($settings['company_tax_number']))<div><b>Tax Company</b>: {{ $settings['company_tax_number'] }}</div>@endif
            @if(!empty($settings['company_address']))<div><b>{{ $isEnglish ? 'Address' : 'Alamat' }}</b>: <span>{!! nl2br(e($settings['company_address'])) !!}</span></div>@endif
            @if(!empty($settings['company_phone']))<div><b>{{ $isEnglish ? 'Phone' : 'Telepon' }}</b>: {{ $settings['company_phone'] }}</div>@endif
    ...
    <table class="summary">
        <tr><td>Subtotal</td><td>{{ $formatMoney($po->total_cost) }}</td></tr>
        <tr class="total"><td>TOTAL</td><td>{{ $formatMoney($po->total_cost) }}</td></tr>
    </table>
