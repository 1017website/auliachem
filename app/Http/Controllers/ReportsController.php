...
class ReportsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $donePOs     = PurchaseOrder::with('items')->whereBetween('order_date', [$startDate, $endDate])->where('currency', 'IDR')->where('status', 'Done')->get();
        $revenue     = $donePOs->sum(fn($po) => $po->total_revenue);
        $totalCost   = $donePOs->sum(fn($po) => $po->total_cost);
        ...
        switch ($reportType) {
            ...
            case 'po':
                $q = PurchaseOrder::with(['customer', 'supplier', 'items'])->whereBetween('order_date', [$startDate, $endDate]);
                if ($status) $q->where('status', $status);
                if ($search) $q->where('po_number', 'like', "%$search%")->orWhereHas('customer', fn($c) => $c->where('company_name', 'like', "%$search%"));
